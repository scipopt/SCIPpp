...
namespace BRANCHING {
    ...
    constexpr Param<bool> COLLECTANCPSCOST { "branching/collectancpscost" };
...
namespace BRANCHING::PSCOST {
    ...
    constexpr Param<double> DISCOUNTFACTOR { "branching/pscost/discountfactor" };
}
...
namespace BRANCHING::RELPSCOST {
    ...
    constexpr Param<double> DYNAMICLOOKAHEADQUOT { "branching/relpscost/dynamiclookaheadquot" };
    ...
    constexpr Param<bool> DYNAMICLOOKAHEAD { "branching/relpscost/dynamiclookahead" };
    ...
    constexpr Param<int> DYNAMICLOOKDISTRIBUTION { "branching/relpscost/dynamiclookdistribution" };
    ...
    constexpr Param<int> MINSAMPLESIZE { "branching/relpscost/minsamplesize" };
    ...
    //! discount factor for ancestral pseudo costs (0.0: disable discounted pseudo costs)
    constexpr Param<double> DISCOUNTFACTOR { "branching/relpscost/discountfactor" };
}
...
namespace CONCURRENT {
    ...
    constexpr Param<bool> SYMMETRYBEFORE { "concurrent/symmetrybefore" };
...
namespace CONFLICT {
    ...
    constexpr Param<bool> USEGENRES { "conflict/usegenres" };
    ...
    constexpr Param<char> REDUCTION { "conflict/reduction" };
    //! should apply the mixed binary reduction?
    constexpr Param<bool> MBREDUCTION { "conflict/mbreduction" };
    ...
    constexpr Param<double> MAXVARSFRACRES { "conflict/maxvarsfracres" };
    ...
    //! number of depth levels up to which first UIP's are used in conflict analysis (-1: use All-FirstUIP rule)
    constexpr Param<int> RESFUIPLEVELS { "conflict/resfuiplevels" };
    ...
    constexpr Param<bool> FIXANDCONTINUE { "conflict/fixandcontinue" };
    ...
    constexpr Param<double> MAXCOEFQUOT { "conflict/maxcoefquot" };
...
namespace CONSTRAINTS::CUMULATIVE {
    ...
    constexpr Param<int> MAXTIME { "constraints/cumulative/maxtime" };
...
namespace CONSTRAINTS::EXACTLINEAR {
    //! frequency for separating cuts (-1: never, 0: only in root node)
    constexpr Param<int> SEPAFREQ { "constraints/exactlinear/sepafreq" };
    //! frequency for propagating domains (-1: never, 0: only in root node)
    constexpr Param<int> PROPFREQ { "constraints/exactlinear/propfreq" };
    //! timing when constraint propagation should be called (1:BEFORELP, 2:DURINGLPLOOP, 4:AFTERLPLOOP, 15:ALWAYS)
    constexpr Param<int> PROPTIMING { "constraints/exactlinear/proptiming" };
    //! frequency for using all instead of only the useful constraints in separation, propagation and enforcement (-1:
    //! never, 0: only in first evaluation)
    constexpr Param<int> EAGERFREQ { "constraints/exactlinear/eagerfreq" };
    //! maximal number of presolving rounds the constraint handler participates in (-1: no limit)
    constexpr Param<int> MAXPREROUNDS { "constraints/exactlinear/maxprerounds" };
    //! should separation method be delayed, if other separators found cuts?
    constexpr Param<bool> DELAYSEPA { "constraints/exactlinear/delaysepa" };
    //! should propagation method be delayed, if other propagators found reductions?
    constexpr Param<bool> DELAYPROP { "constraints/exactlinear/delayprop" };
    //! timing mask of the constraint handler's presolving method (4:FAST, 8:MEDIUM, 16:EXHAUSTIVE, 32:FINAL)
    constexpr Param<int> PRESOLTIMING { "constraints/exactlinear/presoltiming" };
    //! multiplier on propagation frequency, how often the bounds are tightened (-1: never, 0: only at root)
    constexpr Param<int> TIGHTENBOUNDSFREQ { "constraints/exactlinear/tightenboundsfreq" };
    //! maximal number of separation rounds per node (-1: unlimited)
    constexpr Param<int> MAXROUNDS { "constraints/exactlinear/maxrounds" };
    //! maximal number of separation rounds per node in the root node (-1: unlimited)
    constexpr Param<int> MAXROUNDSROOT { "constraints/exactlinear/maxroundsroot" };
    //! maximal number of cuts separated per separation round
    constexpr Param<int> MAXSEPACUTS { "constraints/exactlinear/maxsepacuts" };
    //! maximal number of cuts separated per separation round in the root node
    constexpr Param<int> MAXSEPACUTSROOT { "constraints/exactlinear/maxsepacutsroot" };
    //! apply binaries sorting in decr. order of coeff abs value?
    constexpr Param<bool> SORTVARS { "constraints/exactlinear/sortvars" };
    //! should bounds on continuous variables be tightened by propagation?
    constexpr Param<bool> PROPCONT { "constraints/exactlinear/propcont" };
    //! should denominators of rational bounds on continuous variables be controlled?
    constexpr Param<bool> LIMITDENOM { "constraints/exactlinear/limitdenom" };
    //! maximal denominator for rational bounds on continuous variables after propagation
    constexpr Param<long long> BOUNDMAXDENOM { "constraints/exactlinear/boundmaxdenom" };
}
//! Parameters with prefix constraints/exactsol
namespace CONSTRAINTS::EXACTSOL {
    //! frequency for separating cuts (-1: never, 0: only in root node)
    constexpr Param<int> SEPAFREQ { "constraints/exactsol/sepafreq" };
    //! frequency for propagating domains (-1: never, 0: only in root node)
    constexpr Param<int> PROPFREQ { "constraints/exactsol/propfreq" };
    //! timing when constraint propagation should be called (1:BEFORELP, 2:DURINGLPLOOP, 4:AFTERLPLOOP, 15:ALWAYS)
    constexpr Param<int> PROPTIMING { "constraints/exactsol/proptiming" };
    //! frequency for using all instead of only the useful constraints in separation, propagation and enforcement (-1:
    //! never, 0: only in first evaluation)
    constexpr Param<int> EAGERFREQ { "constraints/exactsol/eagerfreq" };
    //! maximal number of presolving rounds the constraint handler participates in (-1: no limit)
    constexpr Param<int> MAXPREROUNDS { "constraints/exactsol/maxprerounds" };
    //! should separation method be delayed, if other separators found cuts?
    constexpr Param<bool> DELAYSEPA { "constraints/exactsol/delaysepa" };
    //! should propagation method be delayed, if other propagators found reductions?
    constexpr Param<bool> DELAYPROP { "constraints/exactsol/delayprop" };
    //! timing mask of the constraint handler's presolving method (4:FAST, 8:MEDIUM, 16:EXHAUSTIVE, 32:FINAL)
    constexpr Param<int> PRESOLTIMING { "constraints/exactsol/presoltiming" };
    //! should a solution be checked in floating-point arithmetic prior to being processed?
    constexpr Param<bool> CHECKFPFEASIBILITY { "constraints/exactsol/checkfpfeasibility" };
    //! should integrality of continuous implied integral variables be ensured?
    constexpr Param<bool> CHECKCONTIMPLINT { "constraints/exactsol/checkcontimplint" };
    //! fractionality of enforced integral value above which reparation is aborted
    constexpr Param<double> ABORTFRAC { "constraints/exactsol/abortfrac" };
    //! fractionality of weakly implied value up to which reparation fixes variable
    constexpr Param<double> UNFIXFRAC { "constraints/exactsol/unfixfrac" };
    //! maximal number of consecutive repair calls without success
    constexpr Param<int> MAXSTALLS { "constraints/exactsol/maxstalls" };
    //! size of solution buffer
    constexpr Param<int> SOLBUFSIZE { "constraints/exactsol/solbufsize" };
    //! minimal percentage of primal improvement to trigger solution processing
    constexpr Param<double> MINIMPROVE { "constraints/exactsol/minimprove" };
}
...
namespace CONSTRAINTS::KNAPSACK {
    ...
    constexpr Param<bool> COPYTYPEDCONS { "constraints/knapsack/copytypedcons" };
}
...
namespace CONSTRAINTS::LOGICOR {
    ...
    constexpr Param<bool> COPYTYPEDCONS { "constraints/logicor/copytypedcons" };
}
...
namespace CONSTRAINTS::ORBITOPE_FULL {
    //! frequency for separating cuts (-1: never, 0: only in root node)
    constexpr Param<int> SEPAFREQ { "constraints/orbitope_full/sepafreq" };
    //! frequency for propagating domains (-1: never, 0: only in root node)
    constexpr Param<int> PROPFREQ { "constraints/orbitope_full/propfreq" };
    //! timing when constraint propagation should be called (1:BEFORELP, 2:DURINGLPLOOP, 4:AFTERLPLOOP, 15:ALWAYS)
    constexpr Param<int> PROPTIMING { "constraints/orbitope_full/proptiming" };
    //! frequency for using all instead of only the useful constraints in separation, propagation and enforcement (-1:
    //! never, 0: only in first evaluation)
    constexpr Param<int> EAGERFREQ { "constraints/orbitope_full/eagerfreq" };
    //! maximal number of presolving rounds the constraint handler participates in (-1: no limit)
    constexpr Param<int> MAXPREROUNDS { "constraints/orbitope_full/maxprerounds" };
    //! should separation method be delayed, if other separators found cuts?
    constexpr Param<bool> DELAYSEPA { "constraints/orbitope_full/delaysepa" };
    //! should propagation method be delayed, if other propagators found reductions?
    constexpr Param<bool> DELAYPROP { "constraints/orbitope_full/delayprop" };
    //! timing mask of the constraint handler's presolving method (4:FAST, 8:MEDIUM, 16:EXHAUSTIVE, 32:FINAL)
    constexpr Param<int> PRESOLTIMING { "constraints/orbitope_full/presoltiming" };
    //! Whether orbitope constraints should be forced to be copied to sub SCIPs.
    constexpr Param<bool> FORCECONSCOPY { "constraints/orbitope_full/forceconscopy" };
}
//! Parameters with prefix constraints/orbitope_pp
namespace CONSTRAINTS::ORBITOPE_PP {
    //! frequency for separating cuts (-1: never, 0: only in root node)
    constexpr Param<int> SEPAFREQ { "constraints/orbitope_pp/sepafreq" };
    //! frequency for propagating domains (-1: never, 0: only in root node)
    constexpr Param<int> PROPFREQ { "constraints/orbitope_pp/propfreq" };
    //! timing when constraint propagation should be called (1:BEFORELP, 2:DURINGLPLOOP, 4:AFTERLPLOOP, 15:ALWAYS)
    constexpr Param<int> PROPTIMING { "constraints/orbitope_pp/proptiming" };
    //! frequency for using all instead of only the useful constraints in separation, propagation and enforcement (-1:
    //! never, 0: only in first evaluation)
    constexpr Param<int> EAGERFREQ { "constraints/orbitope_pp/eagerfreq" };
    //! maximal number of presolving rounds the constraint handler participates in (-1: no limit)
    constexpr Param<int> MAXPREROUNDS { "constraints/orbitope_pp/maxprerounds" };
    //! should separation method be delayed, if other separators found cuts?
    constexpr Param<bool> DELAYSEPA { "constraints/orbitope_pp/delaysepa" };
    //! should propagation method be delayed, if other propagators found reductions?
    constexpr Param<bool> DELAYPROP { "constraints/orbitope_pp/delayprop" };
    //! timing mask of the constraint handler's presolving method (4:FAST, 8:MEDIUM, 16:EXHAUSTIVE, 32:FINAL)
    constexpr Param<int> PRESOLTIMING { "constraints/orbitope_pp/presoltiming" };
    //! Whether orbitope constraints should be forced to be copied to sub SCIPs.
    constexpr Param<bool> FORCECONSCOPY { "constraints/orbitope_pp/forceconscopy" };
}
...
namespace CONSTRAINTS::SETPPC {
    ...
    constexpr Param<bool> COPYTYPEDCONS { "constraints/setppc/copytypedcons" };
}
...
namespace CONSTRAINTS::VARBOUND {
    ...
    constexpr Param<bool> COPYTYPEDCONS { "constraints/varbound/copytypedcons" };
}
...
//! Parameters with prefix heuristics/dks
namespace HEURISTICS::DKS {
    //! priority of heuristic \<dks\>
    constexpr Param<int> PRIORITY { "heuristics/dks/priority" };
    //! frequency for calling primal heuristic \<dks\> (-1: never, 0: only at depth freqofs)
    constexpr Param<int> FREQ { "heuristics/dks/freq" };
    //! frequency offset for calling primal heuristic \<dks\>
    constexpr Param<int> FREQOFS { "heuristics/dks/freqofs" };
    //! maximal depth level to call primal heuristic \<dks\> (-1: no limit)
    constexpr Param<int> MAXDEPTH { "heuristics/dks/maxdepth" };
    //! maximal number of buckets to be investigated
    constexpr Param<int> MAXBUCKS { "heuristics/dks/maxbucks" };
    //! factor with which the initial kernel size can grow max
    constexpr Param<double> KERNELSIZEFACTOR { "heuristics/dks/kernelsizefactor" };
    //! should a constraint be added ensuring that bucket variables are used?
    constexpr Param<bool> ADDUSECONSS { "heuristics/dks/addUseConss" };
    //! should the linking variables in the kernel influence the size of the buckets?
    constexpr Param<bool> LINKBUCKSIZE { "heuristics/dks/linkbucksize" };
    //! should a variable with different lower bound in transformed and original problem be in the kernel?
    constexpr Param<bool> TRANSLBKERNEL { "heuristics/dks/translbkernel" };
    //! should a variable with max one uplock and one downlock be in the kernel?
    constexpr Param<bool> LESSLOCKSKERNEL { "heuristics/dks/lesslockskernel" };
    //! should dks use the transformed problem?
    constexpr Param<bool> USETRANSPROB { "heuristics/dks/usetransprob" };
    //! defines the maximum mipgap a bucket can be solved to
    constexpr Param<double> BUCKMAXGAP { "heuristics/dks/buckmaxgap" };
    //! defines a bound to the linkscore of the decomp
    constexpr Param<double> MAXLINKSCORE { "heuristics/dks/maxlinkscore" };
    //! defines a maximal share of bin/int variables for a bucket to be respected
    constexpr Param<double> MAXBUCKFRAC { "heuristics/dks/maxbuckfrac" };
    //! maximum number of nodes to regard in all subproblems
    constexpr Param<long long> MAXNODES { "heuristics/dks/maxnodes" };
    //! should a two level bucket structure be used if possible?
    constexpr Param<bool> USETWOLEVEL { "heuristics/dks/usetwolevel" };
    //! should a decomposition be used if given?
    constexpr Param<bool> USEDECOMP { "heuristics/dks/usedecomp" };
    //! should the best solution instead of the LP solution be used?
    constexpr Param<bool> USEBESTSOL { "heuristics/dks/usebestsol" };
    //! should the bucket variables be sorted by reduced costs in the LP solution?
    constexpr Param<bool> REDCOSTSORT { "heuristics/dks/redcostsort" };
    //! should the heuristic terminate after the first primal solution is found?
    constexpr Param<bool> PRIMALONLY { "heuristics/dks/primalonly" };
    //! should the bucket variables be sorted logarithmically by reduced costs in the LP solution?
    constexpr Param<bool> REDCOSTLOGSORT { "heuristics/dks/redcostlogsort" };
    //! should the next solution at least satisfy the old objective?
    constexpr Param<bool> OBJCUTOFF { "heuristics/dks/objcutoff" };
    //! should the heuristic be used only for binary problems or problems with integer and binary variables?
    constexpr Param<bool> RUNBINPROBSONLY { "heuristics/dks/runbinprobsonly" };
}
...
namespace IIS::GREEDY {
    //! priority of iis generation rule \<greedy\>
    constexpr Param<int> PRIORITY { "iis/greedy/priority" };
    //! time limit of optimization process for each individual subproblem
    constexpr Param<double> TIMELIMPERITER { "iis/greedy/timelimperiter" };
    //! node limit of optimization process for each individual subproblem
    constexpr Param<long long> NODELIMPERITER { "iis/greedy/nodelimperiter" };
    //! should an additive constraint approach be used instead of deletion
    constexpr Param<bool> ADDITIVE { "iis/greedy/additive" };
    //! should an unsolved problem (by e.g. user interrupt, node limit, time limit) be considered feasible when deleting
    //! constraints
    constexpr Param<bool> CONSERVATIVE { "iis/greedy/conservative" };
    //! should the deletion routine be performed after the addition routine (in the case of additive)
    constexpr Param<bool> DELAFTERADD { "iis/greedy/delafteradd" };
    //! should satisfied constraints outside the batch of an intermediate solve be added during the additive method
    constexpr Param<bool> DYNAMICREORDERING { "iis/greedy/dynamicreordering" };
    //! the initial batchsize for the first iteration, ignored if initrelbatchsize is positive
    constexpr Param<int> INITBATCHSIZE { "iis/greedy/initbatchsize" };
    //! the initial batchsize relative to the original problem for the first iteration (0.0: use initbatchsize)
    constexpr Param<double> INITRELBATCHSIZE { "iis/greedy/initrelbatchsize" };
    //! the maximum batchsize per iteration
    constexpr Param<int> MAXBATCHSIZE { "iis/greedy/maxbatchsize" };
    //! the maximum batchsize relative to the original problem per iteration
    constexpr Param<double> MAXRELBATCHSIZE { "iis/greedy/maxrelbatchsize" };
    //! the factor with which the batchsize is multiplied in every update
    constexpr Param<double> BATCHINGFACTOR { "iis/greedy/batchingfactor" };
    //! the offset which is added to the multiplied batchsize in every update
    constexpr Param<double> BATCHINGOFFSET { "iis/greedy/batchingoffset" };
    //! the number of iterations to run with a constant batchsize before updating (1: always update)
    constexpr Param<int> BATCHUPDATEINTERVAL { "iis/greedy/batchupdateinterval" };
}
...
namespace MISC {
    ...
    //! bitset describing used symmetry handling technique: (0: off; 1: constraint-based (orbitopes, symresacks);
    //! lexicographic and orbitopal reduction) if dynamic; 2: orbital reduction; 3: orbitopes and symresacks, and
    //! lexicographic/orbital reduction; 4: Schreier Sims cuts; 5: Schreier Sims cuts, orbitopes, symresacks, and/or
    //! lexicographic reduction; 6: Schreier Sims cuts, orbital reduction; 7: Schreier Sims cuts, orbitopes, symresacks,
    //! and/or lexicographic/orbital reduction;) See type_symmetry.h.
...
//! Parameters with prefix presolving/implint
namespace PRESOLVING::IMPLINT {
    //! priority of presolver \<implint\>
    constexpr Param<int> PRIORITY { "presolving/implint/priority" };
    //! maximal number of presolving rounds the presolver participates in (-1: no limit)
    constexpr Param<int> MAXROUNDS { "presolving/implint/maxrounds" };
    //! timing mask of presolver \<implint\> (4:FAST, 8:MEDIUM, 16:EXHAUSTIVE, 32:FINAL)
    constexpr Param<int> TIMING { "presolving/implint/timing" };
    //! should implied integrality also be detected for enforced integral variables?
    constexpr Param<bool> CONVERTINTEGERS { "presolving/implint/convertintegers" };
    //! use the network row addition algorithm when the column to row ratio becomes larger than this threshold
    constexpr Param<double> COLUMNROWRATIO { "presolving/implint/columnrowratio" };
    //! a row that contains variables with coefficients that are greater in absolute value than this limit is not
    //! considered for implied integrality detection
    constexpr Param<double> NUMERICSLIMIT { "presolving/implint/numericslimit" };
}
...
namespace PROPAGATING::SYMMETRY {
    ...
    constexpr Param<bool> DISPSYMINFO { "propagating/symmetry/dispsyminfo" };
    ...
    //! bitset encoding which variable types can be leaders (1: binary; 2: integer; 4: continuous);if multiple types are
    //! allowed, take the one with most affected vars
    ...
    constexpr Param<bool> HANDLESIGNEDORBITOPES { "propagating/symmetry/handlesignedorbitopes" };
    //! Should components consisting of a single full reflection be handled?
    constexpr Param<bool> USESIMPLESGNCOMP { "propagating/symmetry/usesimplesgncomp" };
    ...
    constexpr Param<int> MAXNNEWIVOLUS { "propagating/symmetry/maxnnewivolus" };
    //! terminate symmetry detection using Nauty when depth level of Nauty's search tree exceeds this number (-1:
    //! unlimited)
    constexpr Param<int> NAUTYMAXLEVEL { "propagating/symmetry/nautymaxlevel" };
}
...
namespace RANDOMIZATION {
    ...
    //! multiplier for global shift of all random seeds in the plugins and the LP random seed; this multiplier will be
    //! changed with every SCIP major release
    constexpr Param<int> RANDOMSEEDSHIFTMULTIPLIER { "randomization/randomseedshiftmultiplier" };
...
//! Parameters with prefix reading/nlreader
namespace READING::NLREADER {
    //! should nl files be written in binary format
    constexpr Param<bool> BINARY { "reading/nlreader/binary" };
    //! should comments be written to nl files
    constexpr Param<bool> COMMENTS { "reading/nlreader/comments" };
}
...
namespace READING::OPBREADER {
    ...
    //! maximum allowed 'intsize' (i.e. the number of bits required to represent the sum of absolute values of all
    //! integers that appear in a constraint or objective function) or -1 for unlimited
    constexpr Param<int> MAXINTSIZE { "reading/opbreader/maxintsize" };
}
...
//! Parameters with prefix relaxing/benders
namespace RELAXING::BENDERS {
    //! priority of relaxation handler \<benders\>
    constexpr Param<int> PRIORITY { "relaxing/benders/priority" };
    //! frequency for calling relaxation handler \<benders\> (-1: never, 0: only in root node)
    constexpr Param<int> FREQ { "relaxing/benders/freq" };
    //! continue solving the original SCIP instance if the optimal solution is not found by Benders' decomposition
    constexpr Param<bool> CONTINUEORIG { "relaxing/benders/continueorig" };
    //! the node limit applied only to the Benders' decomposition solve (-1 indicates that the original SCIP node limit
    //! is used).
    constexpr Param<long long> NODELIMIT { "relaxing/benders/nodelimit" };
}
...
namespace SEPARATING::FLOWER {
    //! priority of separator \<flower\>
    constexpr Param<int> PRIORITY { "separating/flower/priority" };
    //! frequency for calling separator \<flower\> (-1: never, 0: only in root node)
    constexpr Param<int> FREQ { "separating/flower/freq" };
    //! maximal relative distance from current node's dual bound to primal bound compared to best node's dual bound for
    //! applying separator \<flower\> (0.0: only on current best node, 1.0: on all nodes)
    constexpr Param<double> MAXBOUNDDIST { "separating/flower/maxbounddist" };
    //! should separator be delayed, if other separators found cuts?
    constexpr Param<bool> DELAY { "separating/flower/delay" };
    //! base for exponential increase of frequency at which separator \<flower\> is called (1: call at each multiple of
    //! frequency)
    constexpr Param<int> EXPBACKOFF { "separating/flower/expbackoff" };
    //! Whether to scan AND constraints when constructing hypergraph
    constexpr Param<bool> SCANAND { "separating/flower/scanand" };
    //! Whether to scan product expressions when constructing hypergraph
    constexpr Param<bool> SCANPRODUCT { "separating/flower/scanproduct" };
    //! Maximum number of standard relaxation inequalities per cut round
    constexpr Param<int> MAXSTANDARD { "separating/flower/maxstandard" };
    //! Maximum number of 1-flower inequalities per cut round
    constexpr Param<int> MAXONEFLOWER { "separating/flower/maxoneflower" };
    //! Maximum number of 2-flower inequalities per cut round
    constexpr Param<int> MAXTWOFLOWER { "separating/flower/maxtwoflower" };
    //! Minimum number of overlaps necessary to try separation
    constexpr Param<int> MINNOVERLAPS { "separating/flower/minnoverlaps" };
    //! Whether to only generate standard inequalities if also flowers were generated
    constexpr Param<bool> DELAYSTANDARD { "separating/flower/delaystandard" };
    //! Number of useless separation rounds after which we stop separating 1-flowers
    constexpr Param<int> MAXUSELESSONEFLOWER { "separating/flower/maxuselessoneflower" };
    //! Number of useless separation rounds after which we stop separating 2-flowers
    constexpr Param<int> MAXUSELESSTWOFLOWER { "separating/flower/maxuselesstwoflower" };
}
...
namespace SOLVINGPHASES {
    ...
    //! should the event handler only determine phase transitions, suppressing settings changes?
...
namespace WRITE {
    ...
    //! should integrality constraints (i.c.) be written for implied integral variables? (0: use original i.c., 1: add
    //! i.c. to strongly implied integral vars, 2: add i.c. to all implied integral vars, -1: remove i.c. from strongly
    //! implied integral vars, -2: remove i.c. from all implied integral vars)
